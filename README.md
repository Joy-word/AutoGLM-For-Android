...
本分支版本新增了**定时任务功能**和**通知触发任务功能**，支持预设任务在指定时间自动执行，以及在收到指定 App 通知时自动触发任务，并提供多种重复模式（每天、工作日、每周等），让手机自动化更加智能和便捷。
...
- ⏰ **定时任务（NEW！！！）**：支持定时执行任务，可设置重复模式，自动亮屏执行
- 🔔 **通知触发（NEW！！！）**：监听指定 App 的通知，收到通知时自动触发预设任务
...
AutoGLM 支持在收到指定 App 的通知时，自动触发预设的任务：

1. 进入「设置」→「通知触发」
2. 授予**通知监听权限**（系统设置 → 通知 → 通知使用权 → 开启 AutoGLM）
3. 点击「添加规则」，选择要监听的 App
4. 输入收到通知后希望自动执行的任务描述
5. 点击「确认」保存规则
6. 规则生效后，每当该 App 推送通知，AutoGLM 即会自动执行对应任务

**通知触发注意事项**：

- 🔔 **通知监听权限**：必须在系统设置中手动授予通知监听权限，AutoGLM 无法自动申请
...
监听「健康」通知 → 打开健康应用，记录今日步数
监听「日历」通知 → 打开备忘录，记录当前日程提醒内容
```
...
1. 确认已在系统设置中授予通知监听权限：系统设置 → 通知 → 通知使用权 → 开启 AutoGLM
2. 确认规则已处于启用状态（规则列表中开关为开启）
3. 检查手机是否设置了电池优化将 AutoGLM 后台进程杀死
4. 部分手机需要在设置中允许 AutoGLM「后台运行」和「自启动」
...
A: 可以在规则列表中关闭对应规则的开关，或者在系统设置中撤销 AutoGLM 的通知监听权限。
